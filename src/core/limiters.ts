...
// A user-provided `languagesMap` takes precedence over the built-in preset.
export function getLanguageLimiters(
  language: string,
  getUserLanguagesMap: () => ILanguagesMapConfig | undefined,
): ILimiters {
  ...
  if (userLanguagesMap !== undefined && userLanguagesMap[language] !== undefined) {
    ...
    return wrapLimiters(currentLangArr[0], currentLangArr[1] || "");
  }

  return getLanguageDefaultLimiters(language);
}
